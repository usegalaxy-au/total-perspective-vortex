...
def setup_destination_mapper(app, tpv_config_files):
    ...
    def reload_destination_mapper(path=None):
        ...
        ACTIVE_DESTINATION_MAPPER = load_destination_mapper(
            tpv_config_files, reload=True
        )

    for tpv_config_file in tpv_config_files:
        if os.path.isfile(tpv_config_file):
            log.info(f"Watching for changes in file: {tpv_config_file}")
            CONFIG_WATCHERS[tpv_config_file] = CONFIG_WATCHERS.get(
                tpv_config_file
            ) or get_watcher(
                app.config, "watch_job_rules", monitor_what_str="job rules"
            )
            CONFIG_WATCHERS[tpv_config_file].watch_file(
                tpv_config_file, callback=reload_destination_mapper
            )
...
def map_tool_to_destination(
    ...
    return ACTIVE_DESTINATION_MAPPER.map_to_destination(
